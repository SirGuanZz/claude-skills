...
提 PR 前的两道闸门，**都只报告不改代码**。先跑综合审查保证基线（安全 / 性能 / 类型 / 测试），需要 Vue 深度复核时再上专项 review。

| Skill | 触发 | 适用阶段 | 关注点 |
|-------|------|---------|------|
| [`pre-pr-review`](./pre-pr-review) | `/pre-pr-review`、"提 PR 前审一下"、"提交前审查" | 提交 PR 前的常规闸门 | 安全性 / 性能瓶颈 / 类型定义完整性 / 测试覆盖 / 最佳实践；默认审当前 diff |
| [`vue-fe-review`](./vue-fe-review) | `/vue-fe-review`、"review 一下"、"前端审一下"、"这代码有啥问题" | 合入前的 Vue 专项复核 | 响应式陷阱 / SSR 反模式 / 路由硬编码跳转 / 页面加载速度 / 内存问题；一轮输出完整结论，自省一次后收口 |

确认 review 结论后再让 agent「按 review 改」。
